...
input double smallLot = 2;
input double minProfit = 5;
input double goodProfit = 250;   // 25% of 1000
...
void OnTick()
{
   ...
   for(int i = 0; i < n; i++)
   {
      if(buy[i] > 0)
      {
          if(OrderSelect(buy[i], SELECT_BY_TICKET))
          {
            if(Profit() >= OrderLots()*goodProfit)
            {
               if(OrderClose(buy[i], OrderLots(), Bid, 100, clrBrown))
               {
                  buy[i] = 0;
                  cur_buy--;
               }
      ...
      if(sell[i] > 0)
      {
         if(OrderSelect(sell[i], SELECT_BY_TICKET))
          {
            if(Profit() >= OrderLots()*goodProfit)
            {
               if(OrderClose(sell[i], OrderLots(), Ask, 100, clrLimeGreen))
               {
                  sell[i] = 0;
                  cur_sell--;
               }
   ...
   if(rsi >= 70)
   {
      ...
      if(ch && cur_sell < n)
      {
         double lotSize = MathMin(smallLot, (1.0*AccountFreeMargin())/1000);
         int ticket = OrderSend(Symbol(), OP_SELL, lotSize, Bid, 100, 0, 0, NULL, 5, 0, clrCrimson); 
         Print("Sold a lot ", lotSize, "free ", AccountFreeMargin(), "ticket ", ticket);
         for(int i = 0; i < n; i++)
         {
            if(sell[i] == 0)
            {
   ...
   if(rsi <= 30)
   {
      ...
      {
         double lotSize = MathMin(smallLot, (1.0*AccountFreeMargin())/1000);
         int ticket = OrderSend(Symbol(), OP_BUY, lotSize, Ask, 100, 0, 0, NULL, 5, 0, clrGreen); 
         Print("Bought a lot ", lotSize, "free ", AccountFreeMargin(), "ticket ", ticket);
         for(int i = 0; i < n; i++)
         {
            if(buy[i] == 0)
            {
